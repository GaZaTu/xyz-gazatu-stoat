...
export class Klipy {
  ...
  async search(q: string, metadata: { customer_id: string, format_filter?: ("gif" | "webp" | "jpg" | "mp4" | "webm")[] }) {
    const url = new URL("gifs/search", this._baseurl)
    url.searchParams.set("q", q)
    url.searchParams.set("customer_id", metadata.customer_id)
    url.searchParams.set("format_filter", metadata.format_filter?.join(",") ?? "gif")
    ...
    const json = await response.json() as {
      data: {
        data: {
          ...
          slug: string
          title: string
          file: Record<"hd" | "md" | "sm" | "xs", Record<"gif" | "webp" | "jpg" | "mp4" | "webm", {
            url: string
            width: number
            height: number
            size: number
